    ...
    <main className="shell">
      <header className="hero">
        ...
        <section className="target-panel">
          <div className="target-row">
            ...
            <button type="button" onClick={() => void handleRemoveTarget()} disabled={!selectedTargetId || selectedTargetId === 'default'} style={{ marginLeft: 8, color: 'red' }}>Remove Server</button>
          </div>
      ...
      {!showTopicPane && (
        <section className="ai-panel" style={{ marginTop: 0 }}>
          <h2>Home: All DB Server Instance Health Overview</h2>
              ...
              <tbody>
                {monitoringStats ? (
                  <tr>
                    ...
                    <td>{formatNumber(monitoringStats.retentionDays)}{monitoringStats.error && <span className="status-warn" title={monitoringStats.error}> ⚠ DB unavailable</span>}</td>
                  </tr>
                ) : (
                  <tr>
                    <td colSpan={7}>Loading monitoring table stats…</td>
                  </tr>
                )}
              </tbody>
            </table>
        ...
        <div className="topic-content">
          {!selectedTopic && (
            <section className="ai-panel topic-placeholder">
              <h2>Select a DBA monitoring domain</h2>
              ...
              <table className="backup-table" style={{ marginTop: 12 }}>
                ...
                <tbody>
                  {historyRows.length > 0 ? (
                    ...
                    <tr>
                      <td colSpan={7}>{historyAuditOnly ? 'No session kill audit events found for selected outcome' : 'No snapshots found'}</td>
                    </tr>
                  )}
                </tbody>
              </table>
          ...
          {selectedTopic === 'security' && (
            <section className="ai-panel">
              ...
              {securityLoading && <p>Loading security data…</p>}
